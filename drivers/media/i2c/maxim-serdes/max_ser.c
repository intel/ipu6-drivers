...
static void __maybe_unused max_ser_i2c_atr_deinit(struct max_ser_priv *priv)
{
...
static bool __maybe_unused max_ser_has_i2c_gate(struct max_ser_priv *priv)
{
				    ...
				    struct v4l2_subdev_state *state)
{
	...
	struct max_ser *ser = priv->ser;
	struct v4l2_subdev_route *route;
	struct max_serdes_vc_remap *local_remaps;
	bool *pipe_set;
	...
	local_remaps = kcalloc(ser->ops->num_pipes, sizeof(*local_remaps),
			       GFP_KERNEL);
	pipe_set = kcalloc(ser->ops->num_pipes, sizeof(*pipe_set), GFP_KERNEL);
	if (!local_remaps || !pipe_set) {
		ret = -ENOMEM;
		goto out_free;
	}
...
static int max_ser_parse_pipe_config(struct max_ser_priv *priv, struct fwnode_handle *fwnode)
{
	...
	fwnode_for_each_child_node(fwnode, child) {
		...
		if (count > 0 && count <= 16) {
			...
			dev_err(priv->dev, "%s: invalid vc-id count %d\n", name, count);
			return -EINVAL;
		}
