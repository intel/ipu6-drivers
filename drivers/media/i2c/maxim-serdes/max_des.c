...
static bool max_des_route_connected(struct max_des_priv *priv,
				    struct v4l2_subdev_route *route)
{
	struct max_des_link *link;

	if (max_des_pad_is_tpg(priv->des, route->sink_pad))
		return true;

	link = max_des_pad_to_link(priv->des, route->sink_pad);
	if (!link)
		return false;

	return max_des_get_link_source(priv, link)->sd != NULL;
}
					...
					struct v4l2_subdev_state *state)
{
	...
	/*
	 * Use the full configured routing, not just connected links, so the
	 * dst VC assigned to a given pipe/phy stays consistent regardless of
	 * which serial links are physically connected.
	 */
					  ...
					  struct v4l2_subdev_state *state)
{
	...
	/* Same as above: compute the dst VC mapping from the full routing. */
	for_each_active_route(&state->routing, route) {
		struct max_des_route_hw hw;
		bool keep_vc;
					 ...
					 enum max_serdes_gmsl_mode mode)
{
	...
	for_each_active_route(&state->routing, route) {
		...
		if (!max_des_route_connected(priv, route))
			continue;
...
static int max_des_select_discovery_links(struct max_des_priv *priv)
{
	struct max_des *des = priv->des;
	unsigned int mask = 0;
	unsigned int i;
	...
	for (i = 0; i < des->ops->num_links; i++) {
		...
		if (link->enabled && !link->probe_failed)
			mask |= BIT(link->index);
	}
				   ...
				   u16 addr, u16 alias)
{
	...
	if (ret) {
		dev_err(priv->dev, "Cannot find serializer for link %u\n",
			link->index);
		link->probe_failed = true;
		if (!link->probe_done) {
			link->probe_done = true;
			if (priv->links_pending)
				priv->links_pending--;
		}
		max_des_select_discovery_links(priv);
		return -ENOENT;
	}
					...
					unsigned int pad)
{
	...
	if (!found) {
		dev_err(priv->dev, "no connected frame desc for source pad %u\n", pad);
		return -ENOENT;
	}
				  ...
				  u32 pad, u64 updated_streams_mask, bool enable)
{
	...
	ret = max_des_populate_mode_context(priv, &mode_context, state,
					    context->mode);
	if (ret)
		goto err_free_context;
			      ...
			      struct v4l2_subdev_state *state)
{
	...
	struct max_des_link *first_enabled = NULL;
	struct max_des_link *connected = NULL;
	...
	for (i = 0; i < des->ops->num_links; i++) {
		...
		source = max_des_get_link_source(priv, link);
		if (!connected && source->sd)
			connected = link;
	}

	/* Prefer a link whose serializer was actually discovered. */
	if (connected)
		first_enabled = connected;

	if (first_enabled) {
		routing.routes[routing.num_routes++] = (struct v4l2_subdev_route) {
			.sink_pad = max_des_link_to_pad(des, first_enabled),
			.sink_stream = 0,
	 ...
	 * If multiple routes are provided, userspace may not be able to
	 * configure them unless the Streams API is enabled. Provide one route.
	 */
	return __max_des_set_routing(sd, state, &routing);
}
...
int max_des_resume(struct max_des *des)
{
	...
	if (des->ops->select_links) {
		...
		if (ret) {
			dev_err(priv->dev,
				"resume: failed to select bound links: %d\n", ret);
			goto err_disable_pocs;
		}
	}
