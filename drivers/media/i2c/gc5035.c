...
/**
	gc5035->mclk = devm_clk_get(dev, "xvclk");
	...
	gc5035->pwdn_gpio = devm_gpiod_get(dev, "powerdown", GPIOD_OUT_HIGH);
	if (IS_ERR(gc5035->pwdn_gpio))
		return dev_err_probe(dev, PTR_ERR(gc5035->pwdn_gpio),
				     "Failed to get pwdn-gpios\n");

	gc5035->resetb_gpio = devm_gpiod_get(dev, "reset", GPIOD_OUT_HIGH);
